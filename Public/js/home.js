...
const addmemberscontainer = document.getElementById(
  "popup-container-addmembers"
);
...
async function getmessages(openedgroupid) {
  try {
    ...
    const result = await axios.get(
      `${IP}/message?lastmessageid=${lastmessageid}&groupid=${openedgroupid}`,
      headers
    );
...
function displaymessage(message, issocket) {
  const li = document.createElement("li");
  if (issocket) {
    chatlist.removeChild(chatlist.getElementsByTagName('li')[0]);
    ...
    li.innerHTML = `${arr[0]} <a href="${arr[1]}">${message.filename}</a>`;
  }
  chatlist.appendChild(li);
}
...
form.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    socket.emit("join-group", openedgroupid);
...
socket.on("recieve-message", (uname, message) => {
    displaymessage(`${uname}: ${message}`, true);
});
  ...
  if (e.target.className === "contact-item1") {
    ...
    if (e.target.value === "true") {
      document.getElementById(
        "chat-heading"
      ).innerHTML = `<h2 id="${e.target.id}">${groupname}</h2>
...
async function getmembers() {
  ...
  const result = await axios.get(
    `${IP}/groupmembers?groupid=${openedgroupid}`,
    headers
  );
...
memberslist.addEventListener("click", async (e) => {
  ...
  if (e.target.className === "remove") {
    ...
    try {
      const result = await axios.get(
        `${IP}/removefromgroup?userid=${userid}&groupid=${openedgroupid}`,
        headers
      );
      getmembers();
    } catch (err) {
    ...
    try {
      const result = await axios.get(
        `${IP}/makeadmin?userid=${userid}&groupid=${openedgroupid}`,
        headers
      );
      getmembers();
    } catch (err) {
    ...
    try {
      const result = await axios.get(
        `${IP}/addmembertogroup?userid=${userid}&groupid=${openedgroupid}`,
        headers
      );
      getmembers();
    } catch (err) {
